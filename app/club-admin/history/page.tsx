      ...
      <div className="relative flex max-h-[90vh] w-full max-w-2xl flex-col rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <div className="mb-3 flex items-center gap-2">
            <h3 className="text-sm font-semibold text-slate-800">Registrations</h3>
            {!loading && (
              <span className="rounded-full bg-[#eef3ff] px-2 py-0.5 text-xs font-bold text-[#1e3a8a]">
                {registrants.length}
              </span>
            )}
          </div>
...
export default function ClubAdminHistoryPage() {
  ...
  const [selectedEvent, setSelectedEvent] = useState<ClubHistoryEvent | null>(null);
  const [error, setError] = useState("");
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (context.error || !context.managedClub) {
        setError(context.error || "No managed club found.");
        setLoading(false);
        return;
      }
      const club = context.managedClub;
      setClubName(club.name?.trim() || club.title?.trim() || "Your Club");

      const { data, error: eventsErr } = await supabase
        .from("events")
        .select("id, title, description, category, event_date, location, approval_status, created_at, is_club_members_only, club_id")
        .eq("club_id", club.id)
        .order("event_date", { ascending: false });

      if (eventsErr) { setError(eventsErr.message); }
      else { setEvents((data ?? []) as ClubHistoryEvent[]); }
      setLoading(false);
    };
    load();
  }, []);
    ...
    <div className="px-8 py-8">
      ...
      <div className="mb-8 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">All events created under your club.</p>
        </div>
        {clubName && (
          <span className="inline-flex items-center rounded-full bg-[#eef3ff] px-4 py-2 text-sm font-semibold text-[#1e3a8a]">
            🏛 {clubName}
          </span>
        )}
      </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-3 py-16 text-center">
            ...
            <p className="text-xs text-slate-400">
              {events.length === 0 ? "No events have been created for this club yet." : "Try adjusting your search or filter."}
            </p>
          </div>
        ) : (
                    ...
                    <tr key={event.id} className="border-b border-slate-50 last:border-0 transition hover:bg-slate-50">
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {event.is_club_members_only && (
                            <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-medium text-blue-700">Members only</span>
                          )}
                        </div>
                        {event.category && (
